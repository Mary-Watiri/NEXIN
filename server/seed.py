...
from app import db
from app import Ticket, TicketStatus, PriorityLevel, Task, Staff
from datetime import datetime, timedelta

# Use the app's context for database operations
with app.app_context():
    # Create a new staff member
    staff_member = Staff(name='John Doe')
    db.session.add(staff_member)
    db.session.commit()

    # Create a new client
    client = Client(name='Client Name', email='client@example.com', phone_number='1234567890')
    ...
    # Create a new task associated with the ticket
    task = Task(title='Task Title', description='Task Description', ticket=new_ticket)

    # Add the ticket and task to the session
    db.session.add_all([new_ticket, task])
    db.session.commit()
